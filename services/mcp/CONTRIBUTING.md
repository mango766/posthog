...
`WithPostHogUrl<T>` for result types, and `withPostHogUrl(data, url)` to add the URL at runtime:
...
export default (): ToolBase<typeof schema, Result> =>
  withUiApp('my-app', {
    ...
    handler: async (context, params) => {
      ...
      return withPostHogUrl({ results: data }, `${context.api.getProjectBaseUrl(projectId)}/my-feature`)
    },
  })
```
