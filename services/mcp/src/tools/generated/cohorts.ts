            ...
            return withPostHogUrl(
                {
                    ...
                    results: (items as any[]).map((item: any) =>
                        withPostHogUrl(item, `${context.api.getProjectBaseUrl(projectId)}/cohorts/${item.id}`)
                    ),
                },
                `${context.api.getProjectBaseUrl(projectId)}/cohorts`
            )
        },
    })
...
const cohortsCreate = (): ToolBase<typeof CohortsCreateSchema, WithPostHogUrl<Schemas.Cohort>> =>
    withUiApp('cohort', {
        ...
        handler: async (context: Context, params: z.infer<typeof CohortsCreateSchema>) => {
            ...
            return withPostHogUrl(
                result as any,
                `${context.api.getProjectBaseUrl(projectId)}/cohorts/${(result as any).id}`
            )
        },
    })
