            ...
            return withPostHogUrl(
                {
                    ...
                    results: (items as any[]).map((item: any) =>
                        withPostHogUrl(
                            item,
                            `${context.api.getProjectBaseUrl(projectId)}/data-management/actions/${item.id}`
                        )
                    ),
                },
                `${context.api.getProjectBaseUrl(projectId)}/data-management/actions`
            )
        },
    })
...
const actionCreate = (): ToolBase<typeof ActionCreateSchema, WithPostHogUrl<Schemas.Action>> =>
    withUiApp('action', {
        ...
        handler: async (context: Context, params: z.infer<typeof ActionCreateSchema>) => {
            ...
            return withPostHogUrl(
                result as any,
                `${context.api.getProjectBaseUrl(projectId)}/data-management/actions/${(result as any).id}`
            )
        },
    })
