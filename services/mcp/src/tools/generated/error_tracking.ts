            ...
            return withPostHogUrl(
                {
                    ...
                    results: (items as any[]).map((item: any) =>
                        withPostHogUrl(item, `${context.api.getProjectBaseUrl(projectId)}/error_tracking/${item.id}`)
                    ),
                },
...
> =>
    withUiApp('error-issue', {
        ...
        handler: async (context: Context, params: z.infer<typeof ErrorTrackingIssuesRetrieveSchema>) => {
            ...
            return withPostHogUrl(
                result as any,
                `${context.api.getProjectBaseUrl(projectId)}/error_tracking/${(result as any).id}`
            )
        },
    })
